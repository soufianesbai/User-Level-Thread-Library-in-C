<<<<<<< Updated upstream:src/thread_sig.c
#include "thread.h"
#include <stddef.h>
...
int thread_sigwait(thread_sigset_t set, int *sig) {
  ...
  while (1) {
    ...
    unsigned int available = self->pending_signals & (unsigned int)set;
    available &= ~self->blocked_signals;
    ...
    if (available != 0) {
      ...
      self->pending_signals &= ~thread_signal_bit(chosen);
      *sig = chosen;
#ifdef ENABLE_PREEMPTION
      preem_unblock();
#endif
    ...
    self->waiting_for_signal = 1;
    self->waited_signals = (unsigned int)set;
    self->state = THREAD_BLOCKED;

    thread *next = thread_scheduler_pick_next();

    if (next == NULL) {
      self->waiting_for_signal = 0;
      self->waited_signals = 0;
