...
int thread_sigwait(thread_sigset_t set, int *sig) {
  ...
  while (1) {
    unsigned int available = current_thread->pending_signals & (unsigned int)set;
    available &= ~current_thread->blocked_signals;
    ...
    if (available != 0) {
      ...
      current_thread->pending_signals &= ~thread_signal_bit(chosen);
      *sig = chosen;
#ifdef ENABLE_PREEMPTION
      preem_unblock();
#endif
    ...
    current_thread->waiting_for_signal = 1;
    current_thread->waited_signals = (unsigned int)set;
    current_thread->state = THREAD_BLOCKED;
    ...
    if (next == NULL) {
      current_thread->waiting_for_signal = 0;
      current_thread->waited_signals = 0;
      current_thread->state = THREAD_RUNNING;
#ifdef ENABLE_PREEMPTION
      preem_unblock();
#endif
