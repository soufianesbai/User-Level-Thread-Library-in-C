...
// The main thread is initialized at the start of the program and will be used as the initial context for the main execution flow.
...
/*
    This function serves as the entry point for new threads. It will call the start function
    with the provided argument and then exit the thread when the function returns.
*/
...
int thread_yield(void)
{
	...
	// To verify !!!
	while ((next = STAILQ_FIRST(&ready_queue)) && next->state == THREAD_TERMINATED) {
